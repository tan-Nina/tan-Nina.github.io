...
在深度强化学习（Deep RL）中，将价值函数的训练从传统的均方误差（MSE）回归问题转化为分类问题（使用分类交叉熵损失），可以显著提高算法的性能、鲁棒性以及在大规模神经网络架构下的可扩展性 。


#### A. 核心方法 (HL-Gauss)
文章主要推崇的方法是 **Histogram Loss with Gaussian Smoothing (HL-Gauss)**。

* **利用序数结构**：这种方法利用了回归任务的序数结构（ordinal structure），不仅仅是将目标离散化，而是通过高斯平滑将概率质量分配给目标值附近的多个“类别”（bins）。
* **对比 Two-Hot**：相比于简单的“Two-Hot”编码（仅将概率分配给最近的两个边界），HL-Gauss 类似于监督学习中的标签平滑（Label Smoothing），能更有效地利用数据。

#### B. 与分布强化学习 (Distributional RL) 的区别
虽然该方法使用了类似于 C51 的分类表示，但它并不一定旨在建模回报的随机分布（Categorical Distributional RL），而是关注于使用分类损失函数（Cross-Entropy）来训练价值函数本身。

---
### 2025，Kevin et al.
本研究并未提出一种全新的算法范式，而是致力于将现有的**对比强化学习（Contrastive Reinforcement Learning, CRL）**框架与现代深度神经网络架构进行系统性融合，以探究深度扩展对强化学习性能的影响。

#### A. 算法基础：对比强化学习(CRL)
本研究使用对比强化学习算法来解决**目标条件强化学习**[👈点击阅读]({% post_url 2025-11-25-RL-post3 %})问题。作为一种**自监督学习（Self-Supervised Learning）**经典算法，CRL运作机制如下：
* **无奖励自主探索（Reward-Free Exploration）：** 在缺乏外部奖励信号的环境中，智能体（Agent）必须通过自主探索机制，习得到达状态空间中任意指定目标（Arbitrary Goals）的策略。
* **基于 InfoNCE 的 Critic 训练：** Critic 网络的优化采用 **InfoNCE 损失函数**[👈点击阅读]({% post_url 2025-11-25-RL-post4 %})。这一公式化处理实际上将原本的价值估计问题重构为**分类任务（Classification Task）**，即判别给定的状态-动作对（State-Action Pair）是否属于通向目标状态的潜在轨迹。

#### B. 网络架构：深度残差网络设计
为了支持网络深度的显著扩展（Scaling），本研究在架构设计上引入了关键的**残差连接（Residual Connections）**机制。

* **残差块微观设计（Residual Block Design）：** 每个残差块由 **4 个重复的计算单元**构成。每个单元内部遵循以下特定的层级序列：
    1.  **Dense Layer**（全连接层）：负责特征的线性变换。
    2.  **LayerNorm**（层归一化）：用于规范化层激活值分布。
    3.  **Swish Activation**（Swish 激活函数）：提供非线性表达能力。
...
在一个 U 型迷宫中，浅层网络（深度 4）非常“短视”。它只看直线距离，即使隔着墙仍然觉得离目标近，结果一直撞墙。而深层网络（深度 64）学会了**状态表示（Representation）**。它理解这不仅是距离的问题，而是需要绕过墙壁才能到达。它的价值函数热力图顺着迷宫的路径延伸 。
...
#### D. 探索与表达能力的协同（Synergy of Exploration & Expressivity）：

实验显示，仅有深层网络（强表达力）若没有好的数据（由浅层收集器提供）是不够的；仅有好的数据（由深层收集器提供）若网络太浅也学不好 。
